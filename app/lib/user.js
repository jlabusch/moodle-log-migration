        ...
        format: {
            'multiple_matches': (row) => {
                ...
                return  "No unique match for \n" +
                        //'\t' + library.add.sql_match(row) + '\n' +
                        "\tid=" + row.userid + ", email='" + row.admin_email + "', username='" + row.admin_username + "'\n" +
                        "\tid=" + row.url.match(/id=(\d+)/)[1] + ", email='" + row.user_email + "', username='" + row.user_username + "'";
            },
            'no_matches': (row) => {
    ...
    "view": {
        ...
        */
        sql_old:    'SELECT log.*, ' +
                    ...
                    (restrict_clause.match(/userid not in/) ?
                        ' AND log.info not in ' + restrict_clause.match(/userid not in\s*(\([^)]+\))/)[1] :
                        ''),
