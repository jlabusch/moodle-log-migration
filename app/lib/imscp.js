...
var library = {
    "add": {
        ...
        info --> mdl_imscp.id & mdl_course_modules.instance 

        */
        sql_old:    'SELECT log.*, ' +
                    '       u.username, u.email, ' +
                    '       c.shortname AS course_shortname, ' +
                    '       x.name AS imscp_name ' +
                    'FROM mdl_log log ' +
                    'JOIN mdl_user u ON u.id = log.userid ' +
                    'JOIN mdl_course c ON c.id = log.course ' +
                    'JOIN mdl_course_modules cm on cm.id = log.cmid ' +
                    'JOIN mdl_imscp x ON x.id = cm.instance and x.course = log.course ' +
                    "WHERE log.module = 'imscp' AND log.action = 'add' AND " + restrict_clause,
        ...
        sql_match:  (row) => {
            return mysql.format(
                ...
                "   (SELECT id from mdl_modules where name = 'imscp') " +
                'WHERE c.shortname = ?',
                [
                    row["username"],
                    row["email"],
    ...
    "view all": { 
        ...
        */
        sql_old:    'SELECT log.*, ' +
                    ...
                    'JOIN mdl_user u ON u.id = log.userid ' +
                    'JOIN mdl_course c ON c.id = log.course ' +
                    "WHERE log.module = 'imscp' AND log.action = 'view all' AND " + restrict_clause,
