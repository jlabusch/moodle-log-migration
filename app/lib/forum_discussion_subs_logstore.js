...
var library = {
    "created": {
        sql_old: `
            ...
            WHERE log.objecttable='forum_discussion_subs' AND log.action='created'
        ...
        sql_old_2pass: (row) => {
            ...
            FROM mdl_forum_discussion_subs fds
            JOIN mdl_forum f ON f.id = fds.forum
            JOIN mdl_forum_discussions fd ON fd.id = fds.discussion
            WHERE fds.id=${row.objectid} `;
            return sql;
        },
        ...
        sql_match:  (row) => {
            return mysql.format(
                ...
                "LEFT JOIN mdl_forum f ON f.name = ? " +
                "LEFT JOIN mdl_forum_discussions fd ON fd.name = ? " +
                "LEFT JOIN mdl_forum_discussion_subs fds ON fds.forum = f.id AND fds.discussion = fd.id AND fds.userid = u.id " +
                "WHERE c.shortname = ? ",
                [
                    row["forum_name"],
                    row["discussion_name"],
    ...
    "deleted": {
        alias: () => { make_alias(library, 'deleted', 'created') }
    }
};
