...
var library = {
    "assigned": {
        sql_old: `
            ...
            WHERE log.objecttable='role' AND log.action='assigned'
        ...
        sql_old_2pass: (row) => {
            let sql= `SELECT r.shortname AS role_shortname
            FROM mdl_role r
            WHERE r.id=${row.objectid} `;
            return sql;
        },
        ...
        sql_match:  (row) => {
            return mysql.format(
                ...
                "LEFT JOIN mdl_role ro ON ro.shortname like ?  " +
                "WHERE c.shortname = ? ",
                [
    ...
    "unassigned": {
        alias: () => { make_alias(library, 'unassigned', 'assigned') }
    },
    'updated':{
        alias: () => { make_alias(library, 'updated', 'assigned') }
    }
};
